...
.config(['$stateProvider', function ($stateProvider) {
    ...
    $stateProvider
        ...
        .state('app.WeightAnalytics.Main', {
            ...
            url: '/main',
            templateUrl: 'Static/weightanalytics_new/main.html',
            controller: 'WeightAnalyticsNewMainCtrl',
        })

        .state('app.WeightAnalytics.WBStatic', {
            url: '/{wb_id:int}/{ws_id:int}?',
            //templateUrl: 'Static/weightanalytics_new/wb_static.html',
            views: {
                "": {
                    templateUrl: 'Static/weightanalytics_new/wb_static.html',
                    controller: 'WeightAnalyticsNewWBCtrl',
                },
                "WS_table@app.WeightAnalytics.WBStatic": {
                    templateUrl: 'Static/weightanalytics_new/ws_table.html',
            ...
            resolve: {
                wb: function (indexService, $stateParams, $state, $q, $timeout) {
                    // need to check of existing WB_ID and WS_ID
...
.controller('WeightAnalyticsNewCtrl', ['$scope', '$rootScope', 'indexService', '$state', 'roles', '$q', 'scalesRefresh', '$interval', '$http', '$filter', '$translate', '$timeout', function ($scope, $rootScope, indexService, $state, roles, $q, scalesRefresh, $interval, $http, $filter, $translate, $timeout) {
    var message = "WeightAnalyticsNewCtrl";
    //alert(message);
...
.controller('WeightAnalyticsNewMainCtrl', ['$scope', '$translate', 'indexService', 'consignersService', '$state', '$interval', '$filter', function ($scope, $translate, indexService, consignersService, $state, $interval, $filter) {
    var message = "WeightAnalyticsNewMainCtrl";
    //alert(message);
...
.controller('WeightAnalyticsNewWBCtrl', ['$scope', '$translate', 'indexService', 'consignersService', 'weightanalyticsService', '$state', '$q', '$timeout', '$interval', '$filter', 'user', 'scalesRefresh', function ($scope, $translate, indexService, consignersService, weightanalyticsService, $state, $q, $timeout, $interval, $filter, user, scalesRefresh) {

    // Init
    var wb_id = $state.params.wb_id;
    var ws_id = $state.params.ws_id;
    var message = "WeightAnalyticsNewWBCtrl" + " \n" + "ID: " + wb_id;
    var Timer = null;
    var isExit = false;
    //alert(message);
    ...
    //$scope.WaybillSelect = vmWaybillSelect;
    $scope.WagonTypeSelect = vmWagonTypeSelect;
    //$scope.WagonNumberSelect = vmWagonNumberSelect;
    //$scope.WagonNumberEnter = vmWagonNumberEnter;
    ...
    function vmGetWSTree() {
        WeightSheetTree = $('#weightsheet_tree').jstree('destroy');
        weightanalyticsService.GetWSList(wb_id).then(function (list) {
            ArchiveWeightSheets = list;
            vmCreateWSTree(ArchiveWeightSheets);
        })
    };


    // загрузка дерева отвесных
    WeightSheetTree.on('loaded.jstree', function (e, data) {
        ...
        // при первой загрузке дерева выделяем год, содержащий последнюю отвесную
    ...
    function vmGetWSInfo(id) {
        ...
        /*
        return $q.all([indexService.getInfo("v_WGT_WeightsheetProperty?$filter=DocumentationsID eq {0} &$orderby=ID".format(id)),
                       indexService.getInfo("v_WGT_Weightsheet?$filter=WeightsheetID eq {0} &$orderby=WaybillNumber,ID".format(id))])
        .then(function (responses) {
            var resp_1 = responses[0].data.value;
            var resp_2 = responses[1].data.value;

            //weightsheet_object.Status = $filter('filter')($scope.ArchiveWeightSheets, { DocumentationsID: id })[0]['Status'];
            weightsheet_object.WeightSheetID = id;
            if (resp_1) {
                for (i = 0; i < resp_1.length; i++) {

                    switch (resp_1[i]['Description2']) {
                        case "Status": {
                            weightsheet_object.Status = resp_1[i]['Value'];
                            break;
                        }
                        case "WeightsheetNumber": {
                            weightsheet_object.WeightSheetNumber = resp_1[i]['Value2'];
                            break;
                        }
                        case "Weigher": {
                            weightsheet_object.Weigher = resp_1[i]['Value2'];
                            break;
                        }
                        case "DocumentType": {
                            var doctype_id = resp_1[i]['Value'];
                            weightsheet_object.WeightingMode = $filter('filter')($scope.WSTypes, { ID: doctype_id })[0];
                            break;
                        }
                        case "SenderShop": {
                            var sender_id = resp_1[i]['Value'];
                            weightsheet_object.SenderShop = $filter('filter')($scope.CargoSenderShops, { ID: sender_id })[0];
                            break;
                        }
                        case "ReceiverShop": {
                            var receiver_id = resp_1[i]['Value'];
                            weightsheet_object.ReceiverShop = $filter('filter')($scope.CargoReceiverShops, { ID: receiver_id })[0];
                            break;
                        }
                        case "StartTime": {
                            //var start_time = new Date(resp_1[i]['Value']);
                            //start_time.setMinutes(start_time.getMinutes() + start_time.getTimezoneOffset());
                            var start_time = resp_1[i]['Value2'];
                            weightsheet_object.CreateDT = start_time;
                            break;
                        }
                        case "EndTime": {
                            //var end_time = new Date(resp_1[i]['Value']);
                            //end_time.setMinutes(end_time.getMinutes() + end_time.getTimezoneOffset());
                            var end_time = resp_1[i]['Value2'];
                            weightsheet_object.EditDT = resp_1[i]['Value'] ? end_time : null;
                            break;
                        }
                        case "Weightbridge": {
                            weightsheet_object.WeightBridgeID = resp_1[i]['Value'];
                            weightsheet_object.WeightBridge = resp_1[i]['Value2'];
                            break;
                        }
    ...
    */
    ...
    function vmCreateWS() {
        ...
        if (['Тарирование', 'Контроль брутто'].indexOf($scope.SelectedObjects.WeightingMode['Description']) == -1 &&
            //if ($scope.SelectedObjects.WeightingMode['Description'] != 'Тарирование' &&
    ...
    function vmTakeWeight() {
        ...
        var CargoTypeID = $scope.SelectedObjects.CargoType.ID;
        CurrentWeight = 65;
    ...
    function vmWaybillSelect(waybill) {

        $scope.SelectedObjects.WagonNumber = null;
        $scope.WagonNumbers = null;
        $scope.SelectedObjects.CargoType = null;
        $scope.SelectedObjects.WagonType = null;
        $scope.CurrentPairNumberTare = null;

        consignersService.GetWaybillObject(waybill['ID'])
        .then(function (waybill_obj) {
            //alert('!!');
            $scope.CurrentWeightSheet.CurrentWeighting['WaybillID'] = waybill['ID'];
            $scope.CurrentWeightSheet.CurrentWeighting['WaybillNumber'] = waybill['WaybillNumber'];
            ...
            if (waybill_obj.WagonNumbers.length > 0) {
                $scope.WagonNumbers = waybill_obj.WagonNumbers;
                $scope.SelectedObjects.WagonNumber = $scope.WagonNumbers[0];
                $scope.CurrentWeightSheet.CurrentWeighting['WagonNumber'] = waybill_obj.WagonNumbers[0]['Description'];
                $scope.CurrentWeightSheet.CurrentWeighting['WagonID'] = waybill_obj.WagonNumbers[0]['ID'];
                // get tare
                vmWagonNumberSelect(waybill_obj.WagonNumbers[0]);
            }
            ////if there is only one wagon in waybill
            //if (waybill_object.WagonNumber) {
            //    $scope.SelectedObjects.WagonNumber = waybill_object.WagonNumber;
            //}
    ...
    */
    ...
    /*
    // выбор номера вагона из списка
    function vmWagonNumberSelect(wagon) {
        //if (!wagon || !wagon['ID']) { return; }
        if (!wagon) { return; }
        // очищаем выбранный вид вагона и тару
        $scope.SelectedObjects.WagonType = null;
        $scope.CurrentPairNumberTare = null;
        // получаем вид вагона и тару для выбранного номера вагона
        $q.all([indexService.getInfo("v_KP4_PackagingUnitsProperty?$filter=ID eq {0}".format(wagon['PackagingUnitsID'])),
                indexService.getInfo("PackagingUnits?$filter=ID eq {0}".format(wagon['PackagingUnitsID']))])
        .then(function (responses) {
            var TareInfo = responses[0].data.value;
            $scope.CurrentPairNumberTare = TareInfo.length ? TareInfo[0].Value : null;
            //alert($scope.CurrentPairNumberTare);
            var wagon_type_id = responses[1].data.value[0] ? responses[1].data.value[0]['PackagingClassID'] : null;
    ...
    // ввод номера вагона (в текст. поле - при тарировании) 
    function vmWagonNumberEnter(wagon) {
        //alert(wagon);
        $scope.CurrentPairNumberTare = null;
        $scope.SelectedObjects.WagonNumber = wagon;
        $scope.CurrentWeightSheet.CurrentWeighting['WagonNumber'] = wagon;
        $scope.CurrentWeightSheet.CurrentWeighting['WagonID'] = null;

        if (!wagon) { return; }

        indexService.getInfo("PackagingUnits?$filter=Description eq '{0}'".format(wagon))
        .then(function (response) {
    ...
    */
    ...
    function vmGetWagonTable(weightsheetid) {
        if ($scope.CurrentWeightSheet && $scope.CurrentWeightSheet.Weighings) {
            //$scope.WeightSheetForPrint = $scope.CurrentWeightSheet.Weighings;
        }
        ...
        .then(function (response) {
            if (response.data.value) {
                ...
                //$scope.$apply();
                //!!!HERE fill CurrentWeightSheet wagons info
            }
        })
        */
    ...
    /*
    // Print after rendering table (if it has rows)
    $scope.ReadyToPrint = function () {
        ...
        // After printing remove WS_toprint ui-view
...
.controller('WeightAnalyticsWSPrintCtrl', ['$scope', '$translate', 'indexService', 'weightanalyticsService', '$state', '$stateParams', '$interval', function ($scope, $translate, indexService, weightanalyticsService, $state, $stateParams, $interval) {
    ...
    $scope.Check = function () {
        //var pppp = $scope;
        //if ($scope.WeightSheetForPrint && !$scope.WeightSheetForPrint.length) {
                ...
                for (i = 0; i < resp_1.length; i++) {
                    ...
                    switch (resp_1[i]['Description2']) {
                        ...
                        case "EndTime": {
                            ...
                            //end_time.setMinutes(end_time.getMinutes() + end_time.getTimezoneOffset());
                            var end_time = resp_1[i]['Value2'];
                            weightsheet_object.EditDT = resp_1[i]['Value2'] ? end_time : null;
                            break;
                        }
                        case "Weightbridge": {
                            weightsheet_object.WeightBridgeID = resp_1[i]['Value'];
                        ...
                        case "Status": {
                            weightsheet_object.Status = resp_1[i]['Value2'];
                            break;
                        }
                        case "Weigher": {
                            weightsheet_object.Weigher = resp_1[i]['Value2'];
                        ...
                        case "DocumentType": {
                            var doctype_id = resp_1[i]['Value'];
                            //weightsheet_object.WeightingMode = $filter('filter')($scope.WSTypes, { ID: doctype_id })[0];
                            weightsheet_object.WeightingMode = { ID: doctype_id };
                            break;
                        }
                        case "SenderShop": {
                            var sender_id = resp_1[i]['Value'];
                            //weightsheet_object.SenderShop = $filter('filter')($scope.CargoSenderShops, { ID: sender_id })[0];
                            weightsheet_object.SenderShop = { ID: sender_id };
                            break;
                        }
                        case "ReceiverShop": {
                            var receiver_id = resp_1[i]['Value'];
                            //weightsheet_object.ReceiverShop = $filter('filter')($scope.CargoReceiverShops, { ID: receiver_id })[0];
                            weightsheet_object.ReceiverShop = { ID: receiver_id };
                            break;
                        }
                    }
                    */
